...
import {
  ...
  Bell, Question, X, CurrencyCircleDollar, TrendUp, Package as Pkg,
  FileText,
...
function ProducerLoading() {
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 60 }}>
      <div style={{
        width: 28, height: 28, borderRadius: "50%",
        border: "3px solid", borderTopColor: "#059669",
        borderRightColor: "rgba(5,150,105,0.2)",
...
const NAV_ITEMS: NavEntry[] = [
  { key: "dashboard", path: "/producer", icon: SquaresFour, labelKey: "nav.farmerDashboard" },
  { key: "lots", path: "/producer/lots", icon: ListDashes, labelKey: "nav.farmerLots" },
  { key: "orders", path: "/producer/orders", icon: Handshake, labelKey: "nav.farmerOrders" },
  { key: "contracts", path: "/producer/contracts", icon: FileText, labelKey: "nav.farmerContracts" },
  { key: "analytics", path: "/producer/analytics", icon: ChartLineUp, labelKey: "nav.farmerAnalytics" },
  { key: "settings", path: "/producer/settings", icon: Gear, labelKey: "nav.farmerSettings" },
];
  ...
  return (
    <div
      ...
      style={{
        ...
        background: active ? colors.accent : hovered ? colors.surfaceHover : "transparent",
    ...
    >
      ...
      {active && !collapsed && (
        <div style={{
          ...
          width: 3, height: 20, borderRadius: "3px 0 0 3px", background: colors.accent,
        }} />
      )}
      {collapsed && hovered && (
        <div style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        color: active ? colors.accent : colors.textMuted, transition: "color 0.15s",
      }}
    >
      <div style={{
...
export default function ProducerLayout() {
  ...
  const allLots = useMemo(() => getAllFarmerLots(), []);
  const myOrders = useMemo(() => orders.filter((o: any) => o.producteurId === user?.id), [orders, user?.id]);
  const activeLots = useMemo(() => allLots.filter((l: any) => l.statut === "Disponible").length, [allLots]);
  const revenue = useMemo(() => myOrders.reduce((s: number, o: any) => s + (o.escrowDeposit ?? 0), 0), [myOrders]);

  const activeKey = NAV_ITEMS.find(
    (item) => item.path === "/producer"
      ? location.pathname === "/producer"
      : location.pathname.startsWith(item.path)
  )?.key;

  const activeLabel = NAV_ITEMS.find((i) => i.key === activeKey)?.labelKey ?? "nav.farmerHub";
  const closeMobile = useCallback(() => setMobileMenu(false), []);

  const crumbs = useMemo(() => {
    const segs = location.pathname.replace("/producer", "").replace(/^\//, "").split("/").filter(Boolean);
    if (segs.length === 0) return [];
    const result: { labelKey: string; path?: string }[] = [
      { labelKey: "nav.farmerDashboard", path: "/producer" },
    ];
    let current = "/producer";
    for (let i = 0; i < segs.length; i++) {
      current += "/" + segs[i];
      if (segs[i] === "lots") result.push({ labelKey: "nav.farmerLots", path: current });
      else if (segs[i] === "orders") result.push({ labelKey: "nav.farmerOrders", path: current });
      else if (segs[i] === "contracts") result.push({ labelKey: "nav.farmerContracts", path: current });
      else if (segs[i] === "analytics") result.push({ labelKey: "nav.farmerAnalytics", path: current });
      else if (segs[i] === "settings") result.push({ labelKey: "nav.farmerSettings", path: current });
      else if (segs[i] === "new") result.push({ labelKey: "nav.new" });
      else if (segs[i] === "edit") result.push({ labelKey: "nav.edit" });
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: colors.bg }}>
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: 1 }}>
            <div style={{
              ...
              background: colors.accentGradient, display: "flex", alignItems: "center", justifyContent: "center",
              flexShrink: 0,
            }}>
            ...
            <div style={{ minWidth: 0, overflow: "hidden" }}>
              ...
              {crumbs.length <= 1 && <div style={{ fontSize: 9, color: colors.accent, fontWeight: 600, lineHeight: 1.2 }}>{t("nav.farmerHub")}</div>}
            </div>
          </div>
        ...
        {mobileMenu && (
          <div style={{
            ...
            <div style={{ borderTop: `1px solid ${colors.borderLight}`, margin: "8px 8px 0", padding: "8px 0" }}>
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          <PageTransition>
            <Suspense fallback={<ProducerLoading />}>
              <Outlet />
            </Suspense>
          </PageTransition>
        </main>
      ...
      <aside style={{
        ...
        <div style={{
          ...
          {!collapsed && (
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              <span style={{ fontSize: 9, color: colors.accent, fontWeight: 600, opacity: 0.8 }}>{t("nav.farmerHub")}</span>
            </div>
          )}
        </div>
        ...
        {!collapsed && (
          <div style={{
            ...
            {[
              { icon: TrendUp, label: t("producer.active"), value: String(activeLots), color: colors.accent },
              { icon: Pkg, label: t("producer.total"), value: String(allLots.length), color: colors.warning },
              { icon: CurrencyCircleDollar, label: t("producer.revenue"), value: formatNumber(revenue), color: colors.info },
            ].map((s) => (
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
            ...
            <span style={{
              ...
              padding: "2px 6px", borderRadius: 4, background: `${colors.accent}15`, color: colors.accent,
            }}>
              {t("nav.farmer")}
            </span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: colors.text, maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {user?.company || user?.email || t("nav.farmer")}
              </span>
            </div>
          </div>
        </header>
